...
4. structured `compose`, `solve`, `inspect`, `render`, and `css` CLI operations;
5. the same five direct MCP tools over the core, with schemas, bounded results,
...
7. a local Figma development plugin that exports one selected node, composes
   ...
   spec needed for in-place replacement;
8. an experimental provider-neutral Capability projection for explicit inspect
   and bounded local render, without a second geometry model;
9. a staged Codex plugin with a thin routing Skill;
...
Warp presets across Rust, CLI, MCP, WASM/Web, and Figma, while keeping
four-corner Distort as the existing direct editor. See
...
- video raster rendering;
- PSD compatibility;
- cloud rendering, accounts, collaboration, marketplace functions;
