...
The persistent icon workspace navigation exposes Perspective, Templates,
Sizes, Mockup, Mesh, and Remap through localized tooltips and sits above the
...
control of that navigation and contains only secondary session, output-density,
...
- at the current Figma panel dimensions, inspect the default Distort surface,
  ...
  confirm More remains the final control of the workspace navigation with its
  Perspective-only entries hidden inside task workspaces, its menu omits active
  Transform commands, the mode strip keeps the checked Transform, Distort, Warp,
  Correct order with Free and Perspective as Distort-local peers, and every
  workspace navigation icon exposes the same localized name by hover and keyboard focus.
