...
| --- | --- |
| Canonical spec and geometry | `crates/worldbend-core/src/model.rs`, `transform.rs`, `solver.rs`, and their tests |
...
| --- | --- | --- |
| One semantic core | CLI, MCP, WASM/Web, Figma, render, CSS, and Capability projections delegate to the same core meaning. | Adapter-local transform math, render shortcut, or diagnostics derived from a second solver. |
...
| Direct routing and catalog economy | Exactly the current five task tools remain directly callable, annotations including `openWorldHint=false` are accurate, ordinary requests need no list/describe pre-call, and live catalog/result/error bytes stay within declared budgets. | A new helper leaks implementation structure, descriptions misroute, catalog growth removes useful schema, or success uses a generic fallback. |
...
plugin version, executable, live five-tool registry, schemas, direct ordinary
...
development regression, runtime Agent flow, Capability conformance/substitution,
