...
try {
  ...
  if(product.length!==1||product[0].licenseDeclared!=='Apache-2.0')throw new Error('Reconcile the product license and artifact notice before packaging');
  for(const file of ['LICENSE','NOTICE']) await cp(path.join(root,file),path.join(component,file));
  const files=[];
  async function inventory(directory,prefix='') {
    for(const entry of await readdir(directory,{withFileTypes:true})) {
      ...
      else if(info.isFile()){const data=await readFile(absolute);if(name.startsWith(`${pluginRoot}/bin/`))assertNoPrivateBuildPaths(data,[root]);files.push({path:name,sha256:hash(data),bytes:data.length,executable:Boolean(info.mode&0o111)});}
  ...
  const legal={license:'LICENSE',notice:'NOTICE',thirdPartyNotices:`${pluginRoot}/THIRD_PARTY_NOTICES.md`,sbom:`${pluginRoot}/sbom/worldbend-macos-arm64.spdx.json`};
  ...
  const report={id:'worldbend',version:plugin.version,archive,archiveSha256:hash(bytes),archiveBytes:bytes.length,descriptorSha256:hash(descriptorBytes),platform:'darwin-arm64',spdx:'Apache-2.0',productLicense:'Apache-2.0',fileCount:files.length};
